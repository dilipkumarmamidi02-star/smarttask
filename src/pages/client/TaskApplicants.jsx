...
export default function TaskApplicants() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const profiles = {};
      apps.forEach((a) => {
        profiles[a.user_email] = { email: a.user_email, full_name: a.user_name };
      });
  ...
  async function handleSubmitReview() {
    ...
    setApplicantReviews((prev) => ({
      ...prev,
      [task.assigned_to]: { rating: reviewForm.rating },
    }));
    setShowReview(false);
    ...
    <div className="max-w-3xl space-y-6">
            ...
            {task.status === "completed" && task.assigned_to && (
              <InvoiceGenerator
                ...
                student={{
                  ...
                  full_name:
                    applicants.find((a) => a.user_email === task.assigned_to)?.user_name,
                }}
              />
            )}
          </div>
      ...
      {showReview && task.assigned_to && (
        <div className="bg-card border border-primary/30 rounded-2xl p-6 space-y-4">
          ...
          <Button onClick={handleSubmitReview} className="font-heading">
            Submit Review
          </Button>
        </div>
      )}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {profile?.college || app.user_email} ·{" "}
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {profile.skills.slice(0, 5).map((s) => (
                          <SkillBadge
                            key={s}
                            skill={s}
                            matched={task.required_skills?.includes(s)}
                          />
                        ))}
                      </div>
                    )}
